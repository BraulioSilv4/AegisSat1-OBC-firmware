...
void init_clocks() {
    // Need a clock for 8 MHz for fast transmission 
    // Try using 32KHz crystal for UART
    // MCLK = 16 MHz (DCO)
    // SCLK = 8 MHz, 4 MHz  (DCO)
    // ACLK = 32 KHz (REFO or XT1) -> (XT1 is more precise, external clock, more delicate than on chip REFO)
    // Configure GPIO
    Init_GPIO();
    ...
    CSCTL1 = DCOFTRIMEN | DCOFTRIM0 | DCOFTRIM1 | DCORSEL_5;// DCOFTRIM=3, DCO Range = 8MHz
